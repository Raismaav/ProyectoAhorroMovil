            ...
            if(!name.getText().toString().equals("") && !lastname.getText().toString().equals("") && !age.getText().toString().equals("") && !sex.getSelectedItem().toString().equals("Selecciona") && !username.getText().toString().equals("") && !passwdUser.getText().toString().equals("")) {
                try {
                    OutputStreamWriter createFileInformationUser = new OutputStreamWriter(openFileOutput(username.getText().toString() + "_credentials.txt", Activity.MODE_PRIVATE));
                        new OutputStreamWriter(openFileOutput("_savings.txt", Activity.MODE_PRIVATE));
                        new OutputStreamWriter(openFileOutput("_investments.txt", Activity.MODE_PRIVATE));
                        new OutputStreamWriter(openFileOutput("_payments.txt", Activity.MODE_PRIVATE));
                        new OutputStreamWriter(openFileOutput("_bills.txt", Activity.MODE_PRIVATE));
                        new OutputStreamWriter(openFileOutput("_incomes.txt", Activity.MODE_PRIVATE));
